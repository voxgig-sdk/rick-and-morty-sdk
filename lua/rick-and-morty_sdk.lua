...
local RickAndMortySDK = {}
RickAndMortySDK.__index = RickAndMortySDK
...
RickAndMortySDK._make_feature = _make_feature


function RickAndMortySDK.new(options)
  local self = setmetatable({}, RickAndMortySDK)
  self.mode = "live"
  self.features = {}
  self.options = nil
...
function RickAndMortySDK:options_map()
...
function RickAndMortySDK:get_utility()
  return Utility.copy(self._utility)
end


function RickAndMortySDK:get_root_ctx()
  return self._rootctx
end


function RickAndMortySDK:prepare(fetchargs)
...
function RickAndMortySDK:direct(fetchargs)
...
-- by passing it.
function RickAndMortySDK:_raw_request(fetchargs)
  local utility = self._utility
...
function RickAndMortySDK:graphql(query, variables, ctrl)
  ...
  if type(errors) == "table" and 0 < #errors then
    ...
    res.ok = false
    res.err = "RickAndMortySDK: graphql: " .. msg
    res.graphql = errors
  end
...
function RickAndMortySDK.test(testopts, sdkopts)
  sdkopts = sdkopts or {}
  ...
  local sdk = RickAndMortySDK.new(sdkopts)
  sdk.mode = "test"
