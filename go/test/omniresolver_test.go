...
import (
	...
	omni "github.com/voxgig-sdk/rick-and-morty-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.RickAndMortyError); is {
				out := map[string]any{
					"name":    "RickAndMortyError",
...
func omniCtx(arg any, client *sdk.RickAndMortySDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.RickAndMortySDK); is {
			client = live
