...
[MCP](https://modelcontextprotocol.io) server exposing the RickAndMorty SDK as
two agent tools — `rick-and-morty_list` and `rick-and-morty_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rick-and-morty-mcp)
make build

# 2. Provide credentials via the environment
export RICK_AND_MORTY_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user rick-and-morty \
  -- /absolute/path/to/rick-and-morty-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./rick-and-morty-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// rick-and-morty_list: first page of records
{ "entity": "character" }
{ "entity": "character", "query": { } }

// rick-and-morty_load: one record by id
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rick-and-morty-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user rick-and-morty \
     -- "$PWD"/dist/*/rick-and-morty-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `rick-and-morty_list` and `rick-and-morty_load` tools now appear
   in new sessions. Ask the agent to *"list character using rick-and-morty"*
   and it calls `rick-and-morty_list` with `{"entity":"character"}`.
...
```sh
export RICK_AND_MORTY_APIKEY=sk_live_xxx            # API key
export RICK_AND_MORTY_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `rick-and-morty_list` tool
...
### Call the `rick-and-morty_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `rick-and-morty_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `rick-and-morty_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `RICK_AND_MORTY_APIKEY` | API key sent with every request. |
| `RICK_AND_MORTY_BASE` | Optional override of the API base URL. |
...
```sh
./rick-and-morty-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"rick-and-morty_load","arguments":{"entity":"character","query":{"id":1}}}}'
```
