...
//     (\`new RickAndMortySDK(...)\` / \`RickAndMortySDK.test(...)\` become
//     \`RickAndMortySDK.test({ entity: ... })\`), transpiled to JS, and run
...
const SDK_NAME = 'RickAndMortySDK'
...
// rewrite real client construction (\`new RickAndMortySDK(...)\` and any
// \`RickAndMortySDK.test(...)\`) to a seeded test-mode client, and give
...
function rewriteForRun(code: string): string {
  ...
  out = out.replace(/new\s+RickAndMortySDK\s*\([^)]*\)/g, () => SEEDED_CTOR)
  out = out.replace(/RickAndMortySDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR)
