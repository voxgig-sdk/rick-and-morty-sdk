...
class FtCtrl extends RickAndMortyControl
{
...
class FtClient
{
    ...
    private RickAndMortyUtility $utility;

    public function __construct(RickAndMortyUtility $utility, array $options)
    {
    ...
    public function get_utility(): RickAndMortyUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (RickAndMortyContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public RickAndMortyContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (RickAndMortyContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = RickAndMortyConfig::make_feature($name);
    ...
    private static function buildUrl(RickAndMortySpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof RickAndMortyError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new RickAndMortyResponse($fetched);
        }
    ...
    private function fail(RickAndMortyContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (RickAndMortyContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
