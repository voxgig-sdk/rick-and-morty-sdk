...
// RickAndMorty SDK struct utility test
//
...
class StructUtilityTest extends TestCase
{
    ...
    {
        if (self::$run === null) {
            $runner = RickAndMortyOmni::makeStructRunner(
        ...
        $this->runsection(
            ...
            function ($input) {
                ...
                if (is_object($store) && $haspath && Struct::undef() !== $val) {
                    ...
                    if (json_encode(RickAndMortyOmni::structfix($now, Struct::undef()))
                        !== json_encode(RickAndMortyOmni::structfix($val, Struct::undef()))) {
