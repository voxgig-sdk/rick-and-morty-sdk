...
class RickAndMortyTestRunner:
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            RickAndMortyTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = RickAndMortyTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = RickAndMortyTestRunner.getenv("RICK_AND_MORTY_TEST_LIVE")
        override = RickAndMortyTestRunner.getenv("RICK_AND_MORTY_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = RickAndMortyTestRunner.getenv(key)
        ...
        explain = RickAndMortyTestRunner.getenv("RICK_AND_MORTY_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if RickAndMortyTestRunner._test_control is not None:
            return RickAndMortyTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                RickAndMortyTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            RickAndMortyTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = RickAndMortyTestRunner.load_test_control()
...
def load_env_local():
    RickAndMortyTestRunner.load_env_local()


def env_override(m):
    return RickAndMortyTestRunner.env_override(m)


def entity_data(v):
    return RickAndMortyTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return RickAndMortyTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return RickAndMortyTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return RickAndMortyTestRunner.load_test_control()


def live_client_options():
    return RickAndMortyTestRunner.live_client_options()


def live_delay_ms():
    return RickAndMortyTestRunner.live_delay_ms()
