...
class TestEpisodeEntity:
    ...
    def test_should_stream(self):
        ...
        base = RickAndMortySDK.test(seed, None)
    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set RICK_AND_MORTY_TEST_EPISODE_ENTID JSON to run live")
        client = setup["client"]
...
def _episode_basic_setup(extra):
    ...
    _entid_env_raw = os.environ.get(
        "RICK_AND_MORTY_TEST_EPISODE_ENTID")
    ...
    idmap_resolved = helpers.to_map(
        env.get("RICK_AND_MORTY_TEST_EPISODE_ENTID"))
    if idmap_resolved is None:
        idmap_resolved = helpers.to_map(idmap)

    if env.get("RICK_AND_MORTY_TEST_LIVE") == "TRUE":
        merged_opts = vs.merge([
            {
            },
            extra or {},
        ])
        client = RickAndMortySDK(helpers.to_map(merged_opts))

    _live = env.get("RICK_AND_MORTY_TEST_LIVE") == "TRUE"
    return {
        "client": client,
        "data": entity_data,
        "idmap": idmap_resolved,
        "env": env,
        "explain": env.get("RICK_AND_MORTY_TEST_EXPLAIN") == "TRUE",
        "live": _live,
