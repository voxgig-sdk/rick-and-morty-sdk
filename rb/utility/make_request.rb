...
module RickAndMortyUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = RickAndMortyResponse.new({})
    ...
    if fetch_err
      ...
      response = RickAndMortyResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = RickAndMortyResponse.new(fetched)
    else
