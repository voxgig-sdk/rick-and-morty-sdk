# RickAndMorty SDK Character entity
...
class CharacterEntity
  ...
  def data_set(args)
    if args
      @_data = RickAndMortyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  def match_set(args)
    if args
      @_match = RickAndMortyHelpers.to_map(VoxgigStruct.clone(args)) || {}
  ...
  # @return [Character, Hash] the loaded Character; raises RickAndMortyError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = RickAndMortyHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
  ...
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<Character>, Array] the matching Character items; raises RickAndMortyError on failure
