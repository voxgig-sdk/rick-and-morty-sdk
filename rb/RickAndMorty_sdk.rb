...
require_relative 'RickAndMorty_types'
...
class RickAndMortySDK
  ...
  def initialize(options = {})
    ...
    feature_opts = RickAndMortyHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = RickAndMortyHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, RickAndMortyFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    RickAndMortyUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = RickAndMortyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = RickAndMortyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = RickAndMortyHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
  ...
  def op_denied(op)
    ...
    {
      "ok" => false,
      "err" => RickAndMortyError.new(
        "#{op}_allow",
        "RickAndMortySDK: #{op}: operation not allowed by" \
  ...
  def raw_request(fetchargs = {})
    ...
    if fetched.is_a?(Hash)
      status = RickAndMortyHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = RickAndMortyError.new(
        "graphql_error", "RickAndMortySDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = RickAndMortySDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
