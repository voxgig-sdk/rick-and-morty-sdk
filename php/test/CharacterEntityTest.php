...
class CharacterEntityTest extends TestCase
{
    ...
    public function test_stream(): void
    {
        ...
        $base = RickAndMortySDK::test($seed, null);
        $seen = iterator_to_array($base->Character(null)->stream("list", null, null), false);
        $this->assertCount(3, $seen);

        // Inbound: streaming active -> yields each item from the feature.
        $cfg = RickAndMortyConfig::make_config();
        if (isset($cfg["feature"]) && is_array($cfg["feature"]) && isset($cfg["feature"]["streaming"])) {
            $sdk = RickAndMortySDK::test($seed, ["feature" => ["streaming" => ["active" => true]]]);
            $got = [];
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set RICKANDMORTY_TEST_CHARACTER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
...
function character_basic_setup($extra)
{
    ...
    $client = RickAndMortySDK::test($options, $extra);
    ...
    $entid_env_raw = getenv("RICKANDMORTY_TEST_CHARACTER_ENTID");
    $idmap_overridden = $entid_env_raw !== false && str_starts_with(trim($entid_env_raw), "{");

    $env = Runner::env_override([
        "RICKANDMORTY_TEST_CHARACTER_ENTID" => $idmap,
        "RICKANDMORTY_TEST_LIVE" => "FALSE",
        "RICKANDMORTY_TEST_EXPLAIN" => "FALSE",
    ]);

    $idmap_resolved = Helpers::to_map(
        $env["RICKANDMORTY_TEST_CHARACTER_ENTID"]);
