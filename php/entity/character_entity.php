...
class CharacterEntity
{
    ...
    {
        if ($args) {
            $this->_data = RickAndMortyHelpers::to_map(Struct::clone($args)) ?? [];
    ...
    {
        if ($args) {
            $this->_match = RickAndMortyHelpers::to_map(Struct::clone($args)) ?? [];
     ...
     *   SDK boundary; throws RickAndMortyError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = RickAndMortyHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
     ...
     *   the SDK boundary; throws RickAndMortyError on failure (item-5 convention).
     */
