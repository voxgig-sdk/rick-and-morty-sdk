...
// RickAndMorty SDK retry feature
...
class RickAndMortyRetryFeature extends RickAndMortyBaseFeature
{
    ...
    public function init(RickAndMortyContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (RickAndMortyContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(RickAndMortyContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(RickAndMortyContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
