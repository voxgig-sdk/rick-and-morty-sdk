...
boru-driven command-line client **and** interactive REPL for the RickAndMorty
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/rick-and-morty-cli)
make build

# 2. See usage (words, entities, env vars)
./rick-and-morty-cli --help
...
./rick-and-morty-cli load 1 character            # {id:1} shorthand
./rick-and-morty-cli load '{id:1}' character       # explicit match map
./rick-and-morty-cli list episode

# 5. Override the API base URL for a single call
RICK_AND_MORTY_BASE=https://api.example.com ./rick-and-morty-cli list character

# 6. No arguments -> interactive REPL
./rick-and-morty-cli
rick-and-morty> list character
rick-and-morty> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/rick-and-morty-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/rick-and-morty-cli list character
   ```
...
```sh
./rick-and-morty-cli load 1 character          # scalar shorthand for {id:1}
./rick-and-morty-cli load '{id:1}' character     # explicit match map
```
...
Run with no arguments to open a REPL (prompt `rick-and-morty>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/rick-and-morty-cli`. |
...
(`./rick-and-morty-cli <expr>`) and interactively (the REPL), and expressions compose the
